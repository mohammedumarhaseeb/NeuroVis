...
for counter in range(config.N_EPOCHS):
    ...
    if config.do_valid:
        ...
        print(
            f"EPOCH {counter}/{config.N_EPOCHS}: Validation average loss: {val_loss/(step+1)} + AUC SCORE = {auc_score}"
        )
        
        if config.OPTIM_THRESHOLD:
            print(f"Optimal Threshold: {best_thresh} with AUC: {auc_score_adj_best}")
            auc_score = auc_score_adj_best
        ...
        if auc_score > best_auc:
            ...
            for f in all_files:
                if f"{config.MODEL_NAME}_{args.type}_fold{args.fold}" in f:
                    os.remove(f"../weights/{f}")

            best_auc = auc_score
            torch.save(
                model.state_dict(),
                f"../weights/3d-{config.MODEL_NAME}_{args.type}_fold{args.fold}_{round(best_auc,3)}.pth",
            )
