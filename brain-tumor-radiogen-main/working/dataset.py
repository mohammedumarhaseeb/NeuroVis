...
class BrainRSNADataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.is_train and config.AUGMENTATION:
            # Basic 3D Augmentations
            aug = T.Compose([
                T.RandRotate90(prob=0.5, spatial_axes=(0, 1)),
                T.RandFlip(prob=0.5, spatial_axis=0),
                T.RandFlip(prob=0.5, spatial_axis=1),
                T.RandFlip(prob=0.5, spatial_axis=2),
                T.RandGaussianNoise(prob=0.2),
            ])
            _3d_images = aug(_3d_images)
